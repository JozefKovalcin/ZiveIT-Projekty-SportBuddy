...
const AddToCalendarDropdown = ({ activity }: { activity: Activity }) => {
  ...
  const generateICS = (activity: Activity) => {
    const lines = [
      ...
      `LOCATION:${(activity.locationName || activity.location || "").replace(/\r?\n/g, "\\n")}`,
  ...
  const googleDates = `${formatForCalendarParam(start)}/${formatForCalendarParam(end)}`;
  ...
  const outlookUrl = `https://outlook.office.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(
    activity.title || ""
  )}&body=${encodeURIComponent(activity.description || "")}&location=${encodeURIComponent(
    activity.locationName || activity.location || ""
  )}&startdt=${encodeURIComponent(start.toISOString())}&enddt=${encodeURIComponent(end.toISOString())}`;
  ...
  const handleDownloadICS = (e: React.MouseEvent) => {
    ...
    element.setAttribute('download', `${(activity.title || "event").replace(/\s+/g, "_")}.ics`);
    ...
    <div className="relative inline-block text-left">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-[color:var(--fluent-card-background)] dark:bg-gray-800 border border-[color:var(--fluent-border)] rounded-lg shadow-xl z-20 overflow-hidden">
            <div className="p-2">
              <div className="text-xs font-semibold text-[color:var(--fluent-text-secondary)] px-3 py-2 uppercase tracking-wider">
                Vybrať kalendár
              </div>
              ...
              <a
                ...
                className="flex items-center gap-3 px-3 py-2.5 text-sm text-[color:var(--fluent-text)] hover:bg-[color:var(--fluent-surface-secondary)] rounded-md transition-colors group"
                onClick={() => setOpen(false)}
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-[color:var(--fluent-text-secondary)]">Otvoriť v prehliadači</div>
                </div>
                <svg className="w-4 h-4 text-[color:var(--fluent-text-secondary)] group-hover:text-[color:var(--fluent-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              >
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-white rounded">
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path fill="#0078D4" d="M22 10.5V20c0 1.1-.9 2-2 2H10.5v-3H22v-8.5z"/>
                    <path fill="#0078D4" d="M22 4v6.5H10.5V2H20c1.1 0 2 .9 2 2z"/>
                    <path fill="#0364B8" d="M10.5 10.5H2V4c0-1.1.9-2 2-2h6.5v8.5z"/>
                    <path fill="#0078D4" d="M10.5 22H4c-1.1 0-2-.9-2-2v-6.5h8.5V22z"/>
                  </svg>
                </div>
                <div className="flex-1">
                  <div className="font-medium">Outlook</div>
                  <div className="text-xs text-[color:var(--fluent-text-secondary)]">Office 365 & Outlook.com</div>
                </div>
                <svg className="w-4 h-4 text-[color:var(--fluent-text-secondary)] group-hover:text-[color:var(--fluent-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                </svg>
              ...
              <div className="my-2 border-t border-[color:var(--fluent-border)]" />

              <button
                onClick={handleDownloadICS}
                className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-[color:var(--fluent-text)] hover:bg-[color:var(--fluent-surface-secondary)] rounded-md transition-colors group"
              >
                <div className="flex-shrink-0 w-8 h-8 flex items-center justify-center bg-[color:var(--fluent-accent)]/10 rounded">
                  <svg className="w-5 h-5 text-[color:var(--fluent-accent)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <div className="flex-1 text-left">
                  <div className="font-medium">Stiahnuť .ics súbor</div>
                  <div className="text-xs text-[color:var(--fluent-text-secondary)]">Pre Apple, iCal a ostatné</div>
                </div>
              </button>
            </div>
          </div>
              ...
              <Card>
                ...
                {showUpcoming && (
                  <CardContent>
                    ...
                    <>
                      {/* Bulk join buttons */}
                      {currentUserId && upcomingInstances.some(inst => 
                        !inst.participations.some(p => p.user.id === currentUserId) && 
                        inst.currentParticipants < inst.maxParticipants
                      ) && (
                        <div className="mb-4 p-4 bg-[color:var(--fluent-surface-secondary)] rounded-lg border border-[color:var(--fluent-border)]">
                          <h4 className="font-semibold text-[color:var(--fluent-text)] mb-3">
                          ...
                          <div className="mb-3">
                            ...
                            <input
                              ...
                              onChange={(e) => setBulkJoinGuestCount(Math.max(0, Math.min(10, parseInt(e.target.value) || 0)))}
                              className="w-24 px-3 py-2 bg-[color:var(--fluent-bg)] border border-[color:var(--fluent-border)] rounded-lg text-[color:var(--fluent-text)] focus:outline-none focus:ring-2 focus:ring-[color:var(--fluent-accent)]"
                            />
                            <span className="ml-2 text-sm text-[color:var(--fluent-text-secondary)]">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <Button
                              ...
                              className="text-sm"
                              onClick={async () => {
                                const totalPerActivity = 1 + bulkJoinGuestCount;
                                if (!confirm(`Prihlásiť sa na všetky dostupné termíny s ${totalPerActivity} ${totalPerActivity === 1 ? 'osobou' : 'osobami'}?`)) return;
                                try {
                                  const response = await fetch(
                                    `${process.env.NEXT_PUBLIC_API_URL}/api/activities/${activity.id}/join-recurring`,
                                    {
                                      ...
                                      credentials: "include",
                                      body: JSON.stringify({ mode: "all", guestCount: bulkJoinGuestCount }),
                                    }
                                  );
                                  if (response.ok) {
                                    const data = await response.json();
                                    ...
                                    const data = await response.json();
                                    alert(data.error || "Chyba pri hromadnom prihlásení");
                                  }
                                } catch (err) {
                                  alert("Chyba pri hromadnom prihlásení");
                                }
                              }}
                            >
                              Prihlásiť na všetky
                            </Button>
                            {activity.recurrenceFrequency === "WEEKLY" && activity.recurrenceDays.length > 1 && (
                              <>
                                {[...new Set(upcomingInstances.map(inst => new Date(inst.date).getDay()))].sort().map(dayNum => {
                                  ...
                                  return (
                                    <Button
                                      ...
                                      onClick={async () => {
                                        const totalPerActivity = 1 + bulkJoinGuestCount;
                                        if (!confirm(`Prihlásiť sa na všetky ${dayNames[dayNum].toLowerCase()} s ${totalPerActivity} ${totalPerActivity === 1 ? 'osobou' : 'osobami'}?`)) return;
                                        try {
                                          const response = await fetch(
                                            `${process.env.NEXT_PUBLIC_API_URL}/api/activities/${activity.id}/join-recurring`,
                                            {
                                    ...
                                    >
                                      Len {dayNames[dayNum].toLowerCase()}
                                    </Button>
                                  );
                                })}
                              </>
            ...
            <Card>
              ...
              <CardContent>
                <Link
                  ...
                  className="flex items-center gap-3 hover:bg-[color:var(--fluent-surface-secondary)] rounded-lg p-2 -m-2 transition-colors cursor-pointer"
                >
