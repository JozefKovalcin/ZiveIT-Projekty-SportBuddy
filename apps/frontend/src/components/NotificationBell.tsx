    ...
    try {
      ...
      if (res.ok) {
        ...
        if (newNotifs.length > 0) {
          ...
          if (lastLatestIdRef.current && latest.id !== lastLatestIdRef.current) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative p-2.5 rounded-lg hover:bg-[color:var(--fluent-surface-secondary)] transition-all duration-200 cursor-pointer"
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 min-w-[20px] h-5 px-1.5 bg-[color:var(--fluent-accent)] rounded-full text-xs font-bold text-white flex items-center justify-center"
      ...
      {isOpen && (
        <div className={`absolute right-0 mt-2 w-96 max-h-[70vh] rounded-xl overflow-hidden z-50 border transition-all duration-300
             ${scrolled 
          ...
          <div className={`px-4 py-3 border-b flex justify-between items-center ${scrolled ? "border-emerald-500/20 bg-emerald-900/20" : "border-white/10 bg-white/5"}`}>
              ...
              notifications.map((n) => (
                <div
                  ...
                  className={`px-4 py-3 border-b cursor-pointer transition-all hover:bg-white/5
                    ...
                    ${!n.read ? (scrolled ? "bg-emerald-500/20" : "bg-emerald-500/10") : ""}`}
                >
                  <div className="flex gap-3">
                    <span className="text-2xl">{getIcon(n.type)}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm ${!n.read ? "text-white font-semibold" : "text-gray-400"}`}>{n.title}</p>
                      <p className="text-xs text-gray-400 truncate">{n.message}</p>
                      <p className="text-xs text-gray-500 opacity-70 mt-1">{timeAgo(n.createdAt)}</p>
                    </div>
      ...
      {selectedNotification && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="relative bg-[#0a0a0a] border border-white/10 rounded-2xl p-6 max-w-md w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200">
              <h3 className="text-xl font-bold text-white mb-2">{selectedNotification.title}</h3>
              <p className="text-gray-300 mb-6 whitespace-pre-wrap">{selectedNotification.message}</p>
              ...
              <div className="flex justify-end gap-3">
                ...
                {selectedNotification.activityId && (
                  <button 
                    onClick={() => {
                      router.push(`/activities/${selectedNotification.activityId}`);
